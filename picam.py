...
filepath = "/home/pi/MOTION"
filenamePrefix = "capture"
diskSpaceToReserve = 800 * 1024 * 1024 # Keep 40 mb free on disk
rotation = 0
cameraSettings = ""
...
#    subprocess.call("raspistill %s -w %s -h %s -t 200 -e jpg -q %s -n -o %s --rotation %s" % (settings, width, height, quality, filename, rotation), shell=True)
    subprocess.call("raspistill %s -w %s -h %s -t 200 -e jpg -q %s -n --rotation %s -o %s" % (settings, width, height, quality, rotation, filename), shell=True)
    print "Captured %s" % filename
    subprocess.call("/bin/true %s" % ("foo"), shell=True)
